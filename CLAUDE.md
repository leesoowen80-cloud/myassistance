...
myassistance — an internal team operations dashboard where members log work items, track statuses, and everyone sees the same live view instead of spreadsheets.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "296526842+leesoowen80-cloud@users.noreply.github.com"
  git config user.name "leesoowen80-cloud"
  ```
